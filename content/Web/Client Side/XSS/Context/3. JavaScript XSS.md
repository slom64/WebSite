...
> [!NOTE] 
> if you have `controlledValue` in `<script> var searchTerms = 'controlledValue'; </script>` then this is exploitable by injecting js command only if the value is inserted as a html from the server, but if it was like `<script>var searchTerms = document.query;</script>` which is variable then you can't escape and inject your js, but still you may be able to exploit this if `searchTerms` used anywhere in the page.
