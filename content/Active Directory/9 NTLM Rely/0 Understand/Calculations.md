...
NTLMv2-Hash            = HMAC-MD5(NT_hash, uppercase(username) + Target Domain) # ResponseKeyNT
...
KeyExchangeKey (KeyEx) = HMAC-MD5(NTLMv2-Hash, Net-NTLMv2 )
EncryptedSessionKey    = Encrypt(RandSKey, KeyEx) # encrypt Client Random number with KeyEx
SessionKey             = If (No KeyExchange) "SessionKey = KeyExchangeKey"; else "Decrypt( EncryptedSessionKey, KeyEx )"# this will give us RandSKey. real word is off
MAC                    = HMAC-MD5(SessionKey, MessageContent + SequenceNumber) # used in signing
```

---
### ⚠️ Prerequisite Values (The Secrets)
...
### Step 1: Calculate the NTLMv2 Hash ($\text{NTOWFv2}$)
...
### Step 3: Calculate the $\text{NTProofValue}$ (The Response Hash)
...
### Step 4: Calculate the $\text{KeyExchangeKey}$ ($\text{KeyEx}$)
...
### Step 5: Calculate the Final $\text{SessionKey}$
...
### 1. Client's Action: Generating the $\text{SessionKey}$ Material
...
### 3. Server's Action: Recovery of the $\text{SessionKey}$
